...
class KnightSpec extends Specification{
  ...
  "A Knight" should{
    val knight = Option(Knight(color = true,moved = false))
    val chessBoard = new ChessBoardFactory().create(3)
    var moveableFields: Vector[(Int,Int)] = Vector()
    chessBoard (0)(0) = knight
    chessBoard (1)(2) = Option(Knight(color = false,moved = false))
    chessBoard (2)(1) = Option(Knight(color = true,moved = false))
